/**
 * TURNERO COOTRABAN v2 — Servidor Nube (Render.com)
 * ==================================================
 * Mejoras v2:
 *  - Registro de tiempos de atención por turno
 *  - Endpoint /reporte para datos JSON del día
 *  - Endpoint /reporte.csv para exportar a Excel
 *  - Soporte completo de métricas por módulo y categoría
 */
...
// ── Estado operativo (turnos activos) ────────────────────────────────────────
let state = {
  queues:      { A: [], T: [], S: [] },
  counters:    { A: 0,  T: 0,  S: 0  },
  calledByMod: { 1:null, 2:null, 3:null, 4:null, 5:null, 6:null },
  lastCalled:  null,
  history:     [],   // {code, del} para la vista TV
};

// ── Registro de tiempos (persiste durante la sesión del servidor) ─────────────
// Cada entrada: { code, cat, mod, issuedAt, calledAt, closedAt, waitSec, attendSec, skipped }
let timeLog = [];

// mapa rápido: code → índice en timeLog
const logIdx = {};
...
const httpServer = http.createServer((req, res) => {
  ...
  if (req.url === '/' || req.url === '/turnero.html') {
    ...
    if (!fs.existsSync(file)) { res.writeHead(404); res.end('turnero.html no encontrado'); return; }
  ...
  // ── Reporte CSV (para abrir en Excel) ────────────────────────────────────
  if (req.url === '/reporte.csv') {
    ...
    res.end('\uFEFF' + csv); // BOM para que Excel abra UTF-8 bien
    return;
  }
...
function buildReport() {
  ...
  const byMod = {};
  atendidos.forEach(t => {
    if (!byMod[t.mod]) byMod[t.mod] = { mod: t.mod, total: 0, avgWait: 0, avgAttend: 0, _w:[], _a:[] };
    byMod[t.mod].total++;
  ...
  return {
    fecha: new Date().toLocaleDateString('es-CO', { dateStyle:'full' }),
    ...
    detalle: timeLog.map(t => ({
      ...
      emitido:    fmtTime(t.issuedAt),
      llamado:    fmtTime(t.calledAt),
      cerrado:    fmtTime(t.closedAt),
...
wss.on('connection', ws => {
  clients.add(ws);
  // Enviar estado actual + reporte inmediato al conectarse
  ...
  ws.on('message', raw => {
    ...
    switch (msg.type) {
      ...
      case 'CALL_NEXT': {
        ...
        // Cerrar turno anterior de este módulo si sigue abierto
      ...
      // ── Operador marca turno como completado ───────────────────────────────
      case 'CLOSE_TURN': {
        const { code, mod } = msg;
      ...
      // ── Eliminar de la cola (sin haber llamado) ────────────────────────────
      case 'REMOVE_QUEUE': {
        const { code, cat } = msg;
      ...
      // ── Reiniciar el día ──────────────────────────────────────────────────
      case 'RESET_ALL': {
        // Archivar log antes de limpiar (en consola para referencia)
        console.log(`[RESET] ${new Date().toISOString()} — ${timeLog.length} turnos del día archivados.`);
        timeLog = [];
...
httpServer.listen(PORT, '0.0.0.0', () => {
  console.log(`\nTurnero Cootraban v2 — puerto ${PORT}`);
  if (APP_URL) console.log(`URL pública: ${APP_URL}`);
  console.log(`Reportes:   ${APP_URL || 'http://localhost:'+PORT}/reporte.csv\n`);
});
